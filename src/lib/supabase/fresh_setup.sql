-- ============================================================
-- FUNDED COBRA SUPPORT PORTAL — FRESH DATABASE SETUP
-- Use this on a brand new Supabase project (no existing tables)
-- ============================================================
...
BEGIN
  IF NEW.ticket_number IS NULL OR NEW.ticket_number = '' THEN
    NEW.ticket_number := 'FC-' || LPAD(NEXTVAL('ticket_number_seq')::TEXT, 5, '0');
  END IF;
  RETURN NEW;
END;
$$;
...
-- tickets
CREATE POLICY "tickets_select" ON tickets FOR SELECT USING (customer_id = auth.uid() OR is_staff());
CREATE POLICY "tickets_insert" ON tickets FOR INSERT WITH CHECK (customer_id = auth.uid());
CREATE POLICY "tickets_update" ON tickets FOR UPDATE USING (is_staff());
...
CREATE POLICY "messages_insert" ON ticket_messages FOR INSERT
  WITH CHECK (sender_id = auth.uid() AND (is_internal = FALSE OR is_staff()));
CREATE POLICY "messages_update" ON ticket_messages FOR UPDATE USING (sender_id = auth.uid() OR is_staff());
CREATE POLICY "messages_delete" ON ticket_messages FOR DELETE USING (sender_id = auth.uid() OR is_staff());

-- internal_notes
CREATE POLICY "notes_select" ON internal_notes FOR SELECT  USING (is_staff());
CREATE POLICY "notes_insert" ON internal_notes FOR INSERT  WITH CHECK (is_staff() AND staff_id = auth.uid());
CREATE POLICY "notes_update" ON internal_notes FOR UPDATE  USING (staff_id = auth.uid());
CREATE POLICY "notes_delete" ON internal_notes FOR DELETE  USING (staff_id = auth.uid() OR is_admin());

-- attachments
CREATE POLICY "attach_select" ON ticket_attachments FOR SELECT
  USING (uploaded_by = auth.uid() OR is_staff() OR EXISTS (
    SELECT 1 FROM tickets WHERE tickets.id = ticket_id AND tickets.customer_id = auth.uid()
  ));
CREATE POLICY "attach_insert" ON ticket_attachments FOR INSERT WITH CHECK (uploaded_by = auth.uid());

-- tags
CREATE POLICY "tags_select" ON ticket_tags FOR SELECT USING (TRUE);
CREATE POLICY "tags_manage" ON ticket_tags FOR ALL   USING (is_staff());

-- tag assignments
CREATE POLICY "tag_assign_all" ON ticket_tag_assignments FOR ALL USING (is_staff());

-- saved replies
CREATE POLICY "saved_select" ON saved_replies FOR SELECT USING (is_staff());
CREATE POLICY "saved_insert" ON saved_replies FOR INSERT WITH CHECK (is_staff() AND staff_id = auth.uid());
CREATE POLICY "saved_update" ON saved_replies FOR UPDATE USING (staff_id = auth.uid() OR is_admin());
CREATE POLICY "saved_delete" ON saved_replies FOR DELETE USING (staff_id = auth.uid() OR is_admin());
...
-- ratings
CREATE POLICY "ratings_select" ON customer_ratings FOR SELECT USING (customer_id = auth.uid() OR is_staff());
CREATE POLICY "ratings_insert" ON customer_ratings FOR INSERT WITH CHECK (customer_id = auth.uid());

-- status history
CREATE POLICY "history_select" ON ticket_status_history FOR SELECT
  USING (is_staff() OR EXISTS (
    SELECT 1 FROM tickets WHERE tickets.id = ticket_id AND tickets.customer_id = auth.uid()
  ));
CREATE POLICY "history_insert" ON ticket_status_history FOR INSERT WITH CHECK (is_staff());
...
-- ── Sync existing auth users + set admin ─────────────────────────
...
UPDATE profiles SET role = 'admin' WHERE email = 'rehanjaved550@gmail.com';
