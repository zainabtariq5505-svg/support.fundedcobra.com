-- ============================================================
-- FUNDED COBRA SUPPORT PORTAL — COMPLETE DATABASE SETUP
-- Run this entire script in Supabase SQL Editor
-- https://supabase.com/dashboard/project/kejymavgfpfliatwgxfu/sql/new
-- ============================================================
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'ticket_number_seq' AND relkind = 'S') THEN
    DROP SEQUENCE ticket_number_seq CASCADE;
  END IF;
END $$;
...
CREATE INDEX idx_status_history_date   ON ticket_status_history(created_at DESC);
...
-- Auto ticket number (FC-10001, FC-10002, …)
...
CREATE POLICY "notif: insert"        ON notifications FOR INSERT WITH CHECK (TRUE);

-- ── customer_ratings ─────────────────────────────────────────────
CREATE POLICY "ratings: insert"      ON customer_ratings FOR INSERT WITH CHECK (customer_id = auth.uid());
CREATE POLICY "ratings: read"        ON customer_ratings FOR SELECT USING (customer_id = auth.uid() OR is_staff());

-- ── ticket_status_history ─────────────────────────────────────────
CREATE POLICY "status_history: read"
...
-- ============================================================
-- This syncs all existing auth users into profiles,
-- then sets rehanjaved550@gmail.com as admin.
...
UPDATE profiles
SET role = 'admin'
WHERE email = 'rehanjaved550@gmail.com';
